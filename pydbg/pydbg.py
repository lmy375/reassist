...
class pydbg:
    ...
    def __init__ (self, ff=True, cs=False):
        ...
        # by Moon 2015/04/15
        self.main_pid                 = 0
        self.processes                = {}        # {process id: process handler}
        self.tid                      = 0
        self.threads                  = {}        # {thread id: thread handler}

        # current pid\h_process\h_thread
    ...
    def attach (self, pid):
        ...
        self.processes[pid] = self.h_process
    ...
    def debug_event_iteration (self):
        ...
        if kernel32.WaitForDebugEvent(byref(dbg), 100):
            ...
            # by Moon 2015/04/15
            # for sub process
            #print "[*] pid:%d tid:%d" % (dbg.dwProcessId, dbg.dwThreadId)
            #print "[*] before"
            #print "[*]Pydbg processes",self.processes
            #print "[*]Pydbg threads",self.threads
            self.pid = dbg.dwProcessId
            if self.processes.has_key(self.pid):
                self.h_process = self.processes[self.pid]
            else:
                self.h_process  = self.open_process(self.pid)
                self.processes[self.pid] = self.h_process

            self.tid = dbg.dwThreadId
            if self.threads.has_key(self.tid):
                self.h_thread = self.threads[self.tid]
            else:
                self.h_thread = self.open_thread(self.tid)
                self.threads[self.tid] = self.h_thread
            #print "[*] after"
            #print "[*]Pydbg processes",self.processes
            #print "[*]Pydbg threads",self.threads
            ...
            # Added by Moon 2015/04/15
            elif dbg.dwDebugEventCode == OUTPUT_DEBUG_STRING_EVENT:
                continue_status = self.event_handler_output_debug_string()

            elif dbg.dwDebugEventCode == RIP_EVENT:
                continue_status = self.event_handler_rip()
            ...
            # by Moon 2015/04/15
            # Don't close. Do this when thread exits.
    ...
    def detach (self):
        ...
        kernel32.DebugActiveProcessStop(self.main_pid)
    ...
    ####################################################################################################################
    # Added by Moon 2015/04/15
    # Just call registered callback function.

    def event_handler_output_debug_string(self):

        if self.callbacks.has_key(OUTPUT_DEBUG_STRING_EVENT):
            return self.callbacks[OUTPUT_DEBUG_STRING_EVENT](self)
        else:
            return DBG_CONTINUE       
        

    def event_handler_rip(self):
        self._err("[!]RIP EVENT! at %#x pid:%d tid:%d", self.context.Eip, self.dbg.dwProcessId, self.dbg.dwThreadId)

        if self.callbacks.has_key(RIP_EVENT):
            return self.callbacks[RIP_EVENT](self)
        else:
            return DBG_CONTINUE
    ...
    def event_handler_exit_process (self):
        ...
        # Modified by Moon 2015/04/15
        # Pydbg stops only if exiting ProcessID equals to main process, 
        # Just ignore subprocess exit exception.
        if self.main_pid == self.dbg.dwProcessId:
            self.set_debugger_active(False)
    ...
    def open_process (self, pid):
        ...
        '''
        # Modified by Moon 2015/04/15
        # This will not set self.h_process
    ...
    def read_process_memory (self, address, length, h_process = None):
        ...
        # Added by Moon 2015/04/15
        # We can't assign value to h_process, or the default value will change!
        _h_process = h_process if h_process else self.h_process

        try:
            old_protect = self.virtual_protect(_address, _length, PAGE_EXECUTE_READWRITE, _h_process)
        except:
            pass

        while length:
            if not kernel32.ReadProcessMemory(_h_process, address, read_buf, length, byref(count)):
                if not len(data):
        ...
        try:
            self.virtual_protect(_address, _length, old_protect, _h_process)
        except:
            pass
    ...
    ####################################################################################################################
    def virtual_protect (self, base_address, size, protection, h_process = None):
        '''
        ...
        if not kernel32.VirtualProtectEx(_h_process, base_address, size, protection, byref(old_protect)):
    ...
    ####################################################################################################################
    def write_process_memory (self, address, data, length=0, h_process = None):
        '''
        ...
        while length:
            ...
            if not kernel32.WriteProcessMemory(_h_process, address, c_data, length, byref(count)):
